<script lang="ts">
  ...
  function confirmDelete() {
    ...
    if (deleteMode === "variable" && pendingVariableId !== null) secrets.deleteVariable(pendingVariableId);
    deleteMode = null;
...
<main class="secrets-page" aria-labelledby="secrets-title">
  <header class="page-header">
    ...
    <Button.Root class="new-project-button" type="button" onclick={() => (projectDialogOpen = true)}>
...
<DeleteConfirmationDialog
  ...
  actionLabel={deleteMode === "project" ? "Delete project" : deleteMode === "environment" ? "Delete environment" : "Delete variable"}
  confirmKeyword={deleteMode === "project" || secrets.selectedEnvironment?.isProduction ? "DELETE" : undefined}
  onOpenChange={(open) => {
...
<style>
  .secrets-page { box-sizing: border-box; display: flex; flex: 1; flex-direction: column; margin: 0 auto; max-width: 1240px; padding: 32px; width: 100%; }
  .page-header, .top-row { align-items: center; display: flex; }
  .page-header { justify-content: space-between; }
  .top-row { gap: 12px; margin-bottom: 8px; }
  .eyebrow { color: var(--color-east-bay-700); font-size: 11px; font-weight: 750; letter-spacing: .06em; margin: 0; text-transform: uppercase; }
  .save-status { align-items: center; background: var(--color-boulder-100); border: 1px solid var(--color-boulder-200); border-radius: 99px; color: var(--color-boulder-600); display: inline-flex; font-size: 11px; font-weight: 650; gap: 5px; padding: 3px 8px; }
  .save-status :global(svg) { color: #15803d; }
  .save-status :global(.spin) { animation: spin 1s linear infinite; color: var(--color-east-bay-700); }
  h1 { color: var(--color-boulder-950); font-size: 30px; letter-spacing: 0; line-height: 1; margin: 0; }
  .page-description { color: var(--color-boulder-600); font-size: 13px; line-height: 1.5; margin: 10px 0 0; max-width: 650px; }
  :global(.new-project-button) { align-items: center; background: var(--color-east-bay-900); border: 0; border-radius: 6px; color: #fff; cursor: pointer; display: inline-flex; font: inherit; font-size: 12px; font-weight: 700; gap: 7px; height: 36px; padding: 0 13px; }
  .error { background: #fef2f2; border: 1px solid #fecaca; border-radius: 6px; color: #b91c1c; font-size: 12px; margin: 20px 0 0; padding: 10px 12px; }
  .workspace { display: grid; gap: 16px; grid-template-columns: minmax(225px, .28fr) minmax(0, .72fr); margin-top: 28px; min-height: 580px; }
  .loading, .empty-workspace { align-items: center; color: var(--color-boulder-500); display: flex; flex: 1; flex-direction: column; font-size: 13px; justify-content: center; min-height: 360px; text-align: center; }
  .empty-workspace h2 { color: var(--color-boulder-900); font-size: 17px; margin: 0; }
  .empty-workspace p { margin: 7px 0 17px; }
  @keyframes spin { to { transform: rotate(360deg); } }
  @media (max-width: 800px) { .secrets-page { padding: 22px 16px; } .page-header { align-items: flex-start; flex-direction: column; gap: 18px; } .workspace { grid-template-columns: 1fr; } .workspace :global(.projects-panel) { max-height: 280px; } :global(.new-project-button) { justify-content: center; width: 100%; } }
</style>
