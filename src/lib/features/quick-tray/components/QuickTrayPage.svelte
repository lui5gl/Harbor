<script lang="ts">
  ...
  import {
    ...
    validateSecretKey,
  } from "$lib/features/secrets/domain/validation";
  import {
    activateSecretProfile,
    loadSecretProfiles,
    saveSecretProfiles,
  } from "$lib/features/secrets/infrastructure/secretsRepository";
  import {
  ...
  async function loadConfiguration() {
    ...
    try {
      const configuration = await loadSecretProfiles();
      projects =
  ...
  async function loadServicesState() {
    ...
    try {
      const [phpInstalled, nodeInstalled, apacheInstalled, running] = await Promise.all([
        getInstalledVersions("PHP"),
        getInstalledVersions("Node.js"),
        getInstalledVersions("Apache"),
        getPhpStatus(),
      ]);
  ...
  async function saveVariable() {
    ...
    const validationError = validateSecretKey(key);
    if (validationError) {
    ...
    if (selectedEnvironment.id === activeEnvironmentId && isNativeApp) {
      try {
        await activateSecretProfile(activeEnvironmentId);
      } catch {
        // non-blocking
      }
    }
  ...
  async function togglePhpService() {
    ...
    try {
      if (isPhpRunning) {
        ...
        await startPhp(phpState.selectedVersion);
        isPhpRunning = true;
  ...
  async function handleNodeVersionChange(version: string) {
    ...
    isServiceBusy = true;
    try {
      await setActiveVersion("Node.js", version);
